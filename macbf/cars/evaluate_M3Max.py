...
# @tf.function(input_signature=[tf.TensorSpec(shape=[num_agents, 4], dtype=tf.float32)])
def build_evaluation_graph(num_agents):
    # s = tf.placeholder(tf.float32, [num_agents, 4])
    s = tf.zeros([num_agents, 4], dtype=tf.float32)  # Replace with your actual initialization
    # g = tf.placeholder(tf.float32, [num_agents, 2])
    g = tf.zeros([num_agents, 2], dtype=tf.float32)  # Replace with your actual initialization
    ...
    def opt_body(a_res, loop_count):
        ...
        error_gradient = tape.gradient(error, a_res)
...
# def build_evaluation_graph(num_agents):
#     s = tf.Variable(tf.zeros([num_agents, 4]), dtype=tf.float32)
#     g = tf.Variable(tf.zeros([num_agents, 2]), dtype=tf.float32)
...
#         deriv = h_next - h + config.TIME_STEP * config.ALPHA_CBF * h
#         deriv = deriv * mask * mask_next
#         error = tf.reduce_sum(tf.maximum(-deriv, 0), axis=1)
#         # error_gradient = tf.gradients(error, a_res)[0]
#         error_gradient = tf.GradientTape(error, a_res)[0]
#         a_res = a_res - config.REFINE_LEARNING_RATE * error_gradient
#         loop_count = loop_count + 1
#         return a_res, loop_count
...
#     #     with tf.GradientTape() as tape:
#     #         tape.watch(a_res)
#     #         error = compute_error(a_res)  # Replace with your actual error computation
#     #     error_gradient = tape.gradient(error, a_res)
#     #     return error_gradient, loop_count + 1
...
def main():
    ...
    checkpoint = tf.train.Checkpoint(vars_restore=vars_restore)
    checkpoint.restore(args.model_path).assert_existing_objects_matched()
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for istep in range(config.EVALUATE_STEPS):
            ...
            # print("s_numpy: ", s.numpy()[:,0])
            # print("g_numpy: ", g.numpy()[:,0])

            # Convert numpy arrays to tuples for use as dictionary keys
            # s_tuple = tuple(s.numpy().flatten())
            # g_tuple = tuple(g.numpy().flatten())
            # s_tuple = tuple(s.numpy())
            # g_tuple = tuple(g.numpy())
            ...
            # s = tf.convert_to_tensor(s_np, dtype=tf.float32)
            # g = tf.convert_to_tensor(g_np, dtype=tf.float32)
            
            # print("s type: ", type(s))
            # print("s_np type: ", type(s_np))
            # print("g type: ", type(g))
            # print("g_np type: ", type(g_np))
            
            # # print("s_tuple type: ", type(s_tuple))
            # # print("s_np type: ", type(s_np))
            # # print("g_tuple type: ", type(g_tuple))
            # # print("g_np type: ", type(g_np))
            
            for i in range(config.INNER_LOOPS):
                a_network, acc_list_np = sess.run([a, acc_list], feed_dict={s: s_np, g: g_np})
                # a_network, acc_list_np = sess.run([a, acc_list], feed_dict={s_tuple: s_np, g_tuple: g_np})
                
                # # Directly call the functions or operations
                # a_network = a(s, g)
                # acc_list_np = acc_list(s, g)
