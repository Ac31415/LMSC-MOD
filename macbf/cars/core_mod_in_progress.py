...
def network_cbf(x, r, indices=None):
    ...
    conv1 = tf.keras.layers.Conv1D(filters=64, kernel_size=1, activation='relu')
    conv2 = tf.keras.layers.Conv1D(filters=128, kernel_size=1, activation='relu')
    conv3 = tf.keras.layers.Conv1D(filters=64, kernel_size=1, activation='relu')
    conv4 = tf.keras.layers.Conv1D(filters=1, kernel_size=1, activation=None)

    # Apply the Conv1D layers
    x = conv1(x)
    x = conv2(x)
    x = conv3(x)
    x = conv4(x)
